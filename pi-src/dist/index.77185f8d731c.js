...
async function fetchVolcengineQuota(baseUrl, accessKeyId, secretAccessKey, request = fetch, plan = "agent") {
...
function inferModelProviderName(modelId) {
...
function resolveProviderMetadata(endpoint, providerHint = "", modelId = "") {
  ...
  const inferredProviderId = !catalog ? inferModelProviderName(modelId) : void 0;
...
var ProjectStatusFooter = class {
  ...
  render(width) {
    ...
    const usageIconWidth = visibleWidth8(usageIconText);
    const usageSegmentsWidth = Math.max(0, contentWidth - usageIconWidth);
    const renderedUsageSegments = usageSegmentsWidth > 0 ? renderStatusLineSegments(usageSegments, usageSegmentsWidth, this.theme.fg("muted", " \xB7 ")) : "";
    const usageLine = usageSegments.length > 0 ? truncateToWidth4(
